...
@end
@interface HSLivePlayVC ()<TXLivePlayListener>{
...
@property (nonatomic, copy) NSString * defaultPlayUrl;
@end

@implementation HSLivePlayVC
- (void)dealloc {
    [self stopPlay];
}
...
- (void)initUI {
   ...
   [self.view setBackgroundImage:[UIImage imageNamed:@"background"]];
    ...
    // 默认播放地址
    _defaultPlayUrl = @"http://liteavapp.qcloud.com/live/liteavdemoplayerstreamid.flv";
}
...
- (void)clickHW:(UIButton *)btn {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 8.0) {
      //  [self toastTip:@"iOS 版本低于8.0，不支持硬件加速."];
        [XSObject showMassage:@"iOS 版本低于8.0，不支持硬件加速."];
        return;
    }
    ...
    if (_btnPlay.tag == 1) {
        if (isHW) {
           // [self toastTip:@"切换为硬解码. 重启播放流程"];
            [XSObject showMassage:@"切换为硬解码. 重启播放流程"];
        }
        else {
           // [self toastTip:@"切换为软解码. 重启播放流程"];
             [XSObject showMassage:@"切换为软解码. 重启播放流程"];
        }
...
//            toast.url = @"https://cloud.tencent.com/document/product/454/7880#RealTimePlay";
            [XSObject showMassage:@"低延时拉流地址需要防盗链签名，详情参考 https://cloud.tencent.com/document/product/454/7880#RealTimePlay"];
            return NO;
        }
        ...
        if ([playUrl hasPrefix:@"rtmp:"]) {
         ...
         //   [self toastTip:@"播放地址不合法，直播目前仅支持rtmp,flv播放方式!"];
             [XSObject showMassage:@"播放地址不合法，直播目前仅支持rtmp,flv播放方式!"];
...
- (BOOL)startPlay {
    ...
    NSString *playUrl = self.defaultPlayUrl;
